...
embedding_decoder = tf.get_variable("embedding_decoder", [target_vocab_size, embedding_size],
                                    dtype=tf.float32)
...
decoder = BasicDecoder(decoder_cell, helper, decoder_cell.zero_state(batch_size, tf.float32).clone(
    cell_state=encoder_state), output_layer=projection_layer)
...
decoder2 = tf.contrib.seq2seq.BasicDecoder(decoder_cell, helper, encoder_state,
